defmodule ChitChat.CustomImageField do
  use Ecto.Type
  def type, do: :string
  ...
  def render_form(conn, changeset, form, field, _options) do
    img_div = if form.data.filename do
      [
        {:safe, ~s(<p><b>Current image:</b> #{form.data.filename}</p>)},
        {:safe, ~s(<div class="form-group">)},
        Phoenix.HTML.Tag.img_tag(Routes.static_url(conn, "/" <> Upload.thumbnail_path(form.data.filename))),
        {:safe, ~s(</div>)}
      ]
    else
      {:safe, ''}
  ...
  def render_index(conn, resource, field, _options) do
    case Map.get(resource, field) do
      ...
      details ->
        filename = details
        [
          Phoenix.HTML.Tag.img_tag(Routes.static_url(conn, "/" <> Upload.thumbnail_path(filename)))
        ]
    end
  end
